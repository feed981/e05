...
<template>
  ...
  <div class="export-category">
    <router-link :to="{ name: 'v2.category.tasks.date', params: { category: `${categoryName}`} }" class="clean-link">
      <i class="fa-solid fa-rocket"></i> : {{ allTaskCount(categoryName, date) }}
    </router-link>

    <router-link :to="{ name: 'v2.category.tasks.date.status', params: { category: `${categoryName}`, date: `${date}`, status: 'normal' } }" class="clean-link">
      <i class="fa-solid fa-person-skiing-nordic"></i> : {{ normalTaskCount(categoryName, date) }}
    </router-link>
    
    <router-link :to="{ name: 'v2.category.tasks.date.status', params: { category: `${categoryName}`, date: `${date}`, status: 'completed' } }" class="clean-link">
      <i class="fa-solid fa-font-awesome"></i> : {{ finishTaskCount(categoryName, date) }}
    </router-link>
    
    <router-link :to="{ name: 'v2.category.tasks.date.status', params: { category: `${categoryName}`, date: `${date}`, status: 'urgent' } }" class="clean-link">
  ...
  <div class="export-container">
    ...
    <div class="empty-message-container">
      <div v-if="props.status === 'normal' && normalTaskCount(categoryName, date) === 0">
        <div class="empty-message">{{ t('task.empty.normal') }}</div>
      </div>
      <div v-if="props.status === 'urgent' && urgentTaskCount(categoryName, date) === 0">
        <div class="empty-message">{{ t('task.empty.urgent') }}</div>
      </div>
      <div v-if="props.status === 'completed' && finishTaskCount(categoryName, date) === 0">
